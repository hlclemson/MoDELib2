...
def main() -> int:
    ...
    folders=['inputFiles','sampledNoises']
    for x in folders:
    ...
    for filename in glob.glob('sampledNoise_*'):
        shutil.move(filename, './sampledNoises/')

    directory = Path('./sampledNoises/')
    numFiles = sum(1 for f in directory.iterdir() if f.is_file())
    ...
    realizations = numFiles
    print(f"realizations = {realizations}")
    sampledNoises = []
    for iterNum in np.arange(1,realizations+1,1):
        ## Iterate over configurations and run one step
        if iterNum==1:
            sampledNoiseField, s_dim = readVTKnoise(f'sampledNoises/sampledNoise_{iterNum}.vtk')
        else:
            sampledNoiseField, _ = readVTKnoise(f'sampledNoises/sampledNoise_{iterNum}.vtk')
        sampledNoises.append(sampledNoiseField)


    sampledNoises = np.array(sampledNoises)
    #sampledNoises = np.mean(sampledNoises, axis=0)
    sampledNoises = np.average(sampledNoises, axis=0)
    #sampledNoises = sampledNoises* (b_SI*mu0_SI)
    ...
    color_i = color_map(inputCorrelationField)

...
def main() -> int:
    ...
    unitVec1_i = unitVec1[:2] # drop the z axis value
    unitVec2_i = unitVec2[:2] 
    ...
    sampledNoiseField, s_dim = readVTKnoise('inputFiles/AlMg5_ISF.vtk')
    #sampledNoiseField = sampledNoiseField*mu0_SI*b_SI
    print(f"np.max(sampledNoiseField) = {np.max(sampledNoiseField)}")
    print(f"np.min(sampledNoiseField) = {np.min(sampledNoiseField)}")

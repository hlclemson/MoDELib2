...
def main() -> int:
    ...
    initialStressGuess = 1
    strSign = -1
    # increase stress by 100 MPa at the first iteration
    searchStepSize = 100
    # Convergence criteria, if the search step size is less than 10 MPa, end the search
    searchResolution = 10 # MPa
    dislocMovingSlowly = False
    dislocationMoving = False
    while not dislocMovingSlowly:
        appliedStr = initialStressGuess
        ...
        # stop the run if the applied stress becomes less than zero
        if appliedStr < 0:
            break

        ...
        for(auto& qPoint : quadraturePoints())
        {
          if((qPoint.slipVectors.first-qPoint.slipVectors.second).squaredNorm()>FLT_EPSILON)
          {
            ...
            {// qPoint is not inside an inclusion, we use the matrix gamma-surface
              ...
              const auto noiseValSfe(slipSystem.gridInterp(qPoint.r-glidePlane.P)); //noise value

              double gammaIsfNoise(0.0);       // ISFNoise
              double gammaSro(0.0);              // The  avg SRO
              double gammaDriftNoise(0.0);  // The drift Noise

              // In the current case we need to consider the effect of sro
              if(slipSystem.planeNoise)
              {
                // Noise For leading partials in the initial state
                gammaIsfNoise=std::get<2>(noiseValSfe);
              // Noise For trailing partials in the initial state
                if ( std::fabs( (qPoint.slipVectors.first).norm() ) >= 0.9999 )
                  //  &&  (std::fabs( (qPoint.slipVectors.first).norm() - std::round((qPoint.slipVectors.first).norm())) < 0.00001) )
                  {
                      gammaDriftNoise = std::get<4>(noiseValSfe) - std::get<2>(noiseValSfe); // SroNoise - ISFNoise
                      gammaSro = -std::get<3>(noiseValSfe); // AvgSro                                                                                       
                  }
                if ( std::fabs( (qPoint.slipVectors.second).norm() ) >= 0.9999 )
                  {
                      gammaDriftNoise = std::get<4>(noiseValSfe) - std::get<2>(noiseValSfe); // SroNoise - ISFNoise
                      gammaSro = std::get<3>(noiseValSfe) ; // AvgSro                                              
                  }
            ...
            //double gammaLePassavg(0.0);
            //double gammaTrPassavg(0.0);
            //gammaIsfavg=gamma2-gamma1; // Initial value for case without noise;
            // if ( std::fabs((qPoint.slipVectors.first).norm())>=0.9999 && (std::fabs( (qPoint.slipVectors.first).norm() - std::round((qPoint.slipVectors.first).norm())) < 0.00001)  )
            //     {

            //         gammaIsfavg = std::get<5>(slipSystem.gridInterp(qPoint.r-glidePlane.P)); 

            //     }

            // For Sitiuation when the full dislocation passed -> glideplane softening trailing
            // if ( std::fabs((qPoint.slipVectors.first).norm())>=1.5000 && (std::fabs( (qPoint.slipVectors.second).norm() - std::round((qPoint.slipVectors.second).norm())) < 0.00001)  )
            //     {
            //         gammaSroNoise = std::get<3>(slipSystem.gridInterp(qPoint.r-glidePlane.P));
            //         gammaDriftNoise = gammaSroNoise - gammaIsfNoise; // SroNoise - ISFNoise

            //         gammaTrPassavg = std::get<6>(slipSystem.gridInterp(qPoint.r-glidePlane.P)); 
            //         gammaIsfavg = 0.0;
            //         gammaIsfNoise=0.0;        
            //     }

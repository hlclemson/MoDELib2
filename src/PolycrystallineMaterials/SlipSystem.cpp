...
std::tuple<Eigen::Matrix<double,3,3>,double,double,double,double> SlipSystem::gridInterp(const VectorDim& x) const
{   // Added by Hyunsoo (hyunsol@g.clemson.edu)
  ...
  if(planeNoise)
  {
    ...
    const std::tuple<double,double,double,double,double> gridNoise(planeNoise->gridInterp(globalToLocal(x) ));
    ...
    return std::make_tuple(ssStress,ssRSS,std::get<2>(gridNoise),std::get<3>(gridNoise),std::get<4>(gridNoise));
  }
  else
  {
    return std::make_tuple(Eigen::Matrix<double,3,3>::Zero(),0.0,0.0,0.0,0.0);
  }
}

std::tuple<Eigen::Matrix<double,3,3>,double,double,double,double> SlipSystem::gridVal(const Eigen::Array<int,2,1>& idx) const
{   // Added by Hyunsoo (hyunsol@g.clemson.edu), and XinLiu liuxin23@mails.tsinghua.edu.cn

  if(planeNoise)
  {
    const std::tuple<double,double,double,double,double> gridNoise(planeNoise->gridVal(idx));

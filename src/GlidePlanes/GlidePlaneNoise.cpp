  ...
  for(const auto& pair : noiseFiles)
  {
    ...
    if(noiseFileString!="none" && noiseFileString!="None")
    {
      ...
      if(type=="MDSolidSolutionNoise")
      {
        ...
        const auto success(solidSolutionNoise().emplace(tag,new MDSolidSolutionNoise(mat,type,tag,correlationFile_xz,correlationFile_yz,seed,gridSize,gridSpacing,Eigen::Matrix<double,2,2>::Identity(),a_Cai)));
        if(!success.second)
        {
          throw std::runtime_error("Could not insert noise "+tag);
        }

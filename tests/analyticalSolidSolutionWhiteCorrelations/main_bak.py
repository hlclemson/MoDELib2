...
def set_initial_configuration() -> None:
    ...
    an_noise_fname = str(config.files_to_copy_from_lib['noise_file_an_ssw']).split('/')[-1]
    modify_dict_parameters(config.AnalyticalSolidSolutionWhite_parameters, an_noise_fname)
...
def main() -> int:
    ...
    noise_seed_to_test = [2]
    node_num_to_test = [ 2860 ]
    ...
    for seed in noise_seed_to_test:
        for node in node_num_to_test:
            ...
            while not dislocMovingSlowly:
                ...
                modify_parameter('AnalyticalSolidSolutionWhiteNoise.txt', 'seed', seed)

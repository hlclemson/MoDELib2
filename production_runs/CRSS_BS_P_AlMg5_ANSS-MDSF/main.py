...
def main() -> int:
    ...
    for seed in noise_seed_to_test:
        ...
        initial_search_step_size = 100
        dislocMovingSlowly = False
        dislocationMoving = False
        while not dislocMovingSlowly:
            # initial CRSS search step size of bisection search
            ...
            target_dir = (
                dataStorageDir / f"seed{seed}" / f"{stress}MPa".replace("-", "")
            )
            ...
            f = open(target_dir / "../search_log.txt", "a")
            f.write(f"{stress}MPa, seed{seed}\n")

            if dydx_mean > glidePlasticStrain:
                # while loop stop condition
                if initial_search_step_size < crss_search_resolution:
                    dislocMovingSlowly = True
                    f.write(
                        f"found CRSS, {stress-initial_search_step_size}MPa, seed{seed}"
                    )
                else:
                    dislocationMoving = True
                    initial_search_step_size -= initial_search_step_size // 2
                    stress -= initial_search_step_size
            else:
                if (
                    initial_search_step_size < crss_search_resolution
                    and dislocationMoving
                ):
                    dislocMovingSlowly = True
                    f.write(
                        f"found CRSS, {stress-initial_search_step_size}MPa, seed{seed}"

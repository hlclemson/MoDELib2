...
def main():
    ...
    with os.scandir(dataDir) as seed_dir:
        for i in seed_dir:
            if i.is_dir() and "node" in i.name:
                node_data_dir.append(i.name)
    node_data_dir = sorted(node_data_dir, key=lambda x: int(x.replace('node', '')))

    record_data_output = []
    for node in node_data_dir:

        local_data_dir = Path(dataDir)/node
        stress_data_dir = []
        with os.scandir(local_data_dir) as dirs:
            for i in dirs:
        ...
        for stress_dir in stress_data_dir:
            ddd_data_dir = local_data_dir/stress_dir
            ...
            #runID = runIDs[-1]
                ...
                for loopID, network_nodes in loop_nodes_per_loopID.items():
                    ...
                    #print(evl_nodes.nodes[mask, get_idx('VELOCITY_COLS_IDX_IN_NODE')]*b_SI/convertTimeUnit)
                    #print(np.mean(evl_nodes.nodes[mask, get_idx('VELOCITY_COLS_IDX_IN_NODE')], axis=0)*b_SI/convertTimeUnit)
                    #print((np.linalg.norm(evl_nodes.nodes[mask, get_idx('VELOCITY_COLS_IDX_IN_NODE')]))*b_SI/convertTimeUnit)

                    # extract each nodal velocity (1x3) and average over nodes
                    all_node_velocity = evl_nodes.nodes[mask, get_idx('VELOCITY_COLS_IDX_IN_NODE')]

                    # average over all nodes and calc velocity vector magnitude
                    node_velocity_per_loopID[loopID] = np.linalg.norm(np.mean(all_node_velocity, axis=0))*b_SI/convertTimeUnit
            ...
            # convert all runID velocity data to np.array
            record_data_mean_temp =np.array(record_data_mean_temp)
            # calculate the mean velocity over recorded runIDs
            record_data_mean_temp = np.mean(np.array(record_data_mean_temp), axis=0)

            record_data_temp = [node, int(stress_dir.replace('MPa', ''))]
            record_data_temp.extend(list(record_data_mean_temp))
            record_data_output.append(record_data_temp)

    # Define column headers
    headers = ['node', 'Stress', 'mean_velocity_plane1', 'mean_velocity_plane2']
    ...
    df.to_csv(Path(output_dir)/'velocity_data.csv', index=False)
    print(df)
    ...
    print(df_avg)
    df_avg.to_csv(Path(output_dir)/'velocity_data_avg.csv', index=False)
    exit()

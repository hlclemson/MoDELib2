...
def main():
    ...
    output_dir = Path(dataDir)/"dislocation_self_energy"
    ...
    for runID in runIDs:
        ...
        aux =readAUXtxt(str(Path(dataDir)/'evl'/f'ddAux_{runID}'))
        gauss_data = np.array(aux.gaussPoints)

        # Convert to NumPy arrays
        source_sink_pairs = gauss_data[:, 0:2]
        # init dictionary
    ...
    print(t_elastic_energies)
    # convert to dictionary
    #t_elastic_energies = {int(loop_id): np.array(energies) for loop_id, energies in t_elastic_energies.items()}
    #t_core_energies = {int(loop_id): np.array(energies) for loop_id, energies in t_core_energies.items()}
    #t_total_energies = {
    #loop_id: t_elastic_energies[loop_id] + t_core_energies[loop_id]
    #for loop_id in t_elastic_energies.keys() & t_core_energies.keys()
    #}

    avg_over_loop_elastic_e = np.array(list(t_elastic_energies.values()))
    print(avg_over_loop_elastic_e)
    exit()

    print()
    # Convert dictionaries to DataFrames
    df_elastic = pd.DataFrame.from_dict(t_elastic_energies, orient='index').T  # Transpose to have loop IDs as columns
    exit()
    df_core = pd.DataFrame.from_dict(t_core_energies, orient='index').T
    df_total_E = pd.DataFrame.from_dict(t_total_energies, orient='index').T

    # save the last data point
    elastic_last_runID = df_elastic.iloc[-1]
    # Save with a custom header
    elastic_last_runID.to_csv(
        Path(output_dir)/'converged_elastic_energies.csv',
        index_label='Loop_ID',
        header=['Energy [J]']  # Rename the value column
    )

    core_last_runID = df_core.iloc[-1]
    # Save with a custom header
    core_last_runID.to_csv(
        Path(output_dir)/'converged_core_energies.csv',
        index_label='Loop_ID',
        header=['Energy [J]']  # Rename the value column
    )

    total_E_last_runID = df_total_E.iloc[-1]
    # Save with a custom header
    total_E_last_runID.to_csv(
        Path(output_dir)/'converged_total_energies.csv',
        index_label='Loop_ID',
        header=['Energy [J]']  # Rename the value column
    )

    # convergence check
    plot_data(runIDs, t_elastic_energies, 'elastic energy', Path(output_dir)/'elastic_energy.png')
    plot_data(runIDs, t_core_energies, 'core energy', Path(output_dir)/'core_energy.png')
    plot_data(runIDs, t_total_energies, 'total energy', Path(output_dir)/'total_energy.png')

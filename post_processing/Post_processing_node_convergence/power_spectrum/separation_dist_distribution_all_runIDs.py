...
def main():
    ...
    for runID in runIDs:
        ...
        #fig, axes = plt.subplots(1, 2, figsize=(10, 6))  # 2x2 grid (adjust to 4x4 if needed)
        ...
        ## Plot each loop's data
        #for ax, partial_sep_dist in zip(axes, partial_dists_all_planes):
        #    mean_val = np.mean(partial_sep_dist)
        #    std_val = np.std(partial_sep_dist)
        #    #ax.plot(runIDs, en, 'o-')
        #    ax.hist(partial_sep_dist, bins='auto', density=True, alpha=0.3)
        ...
        #os.makedirs(Path(dataDir)/"separation_dist", exist_ok=True)
        #fig.savefig(Path(dataDir)/"separation_dist"/f"separation_{runID}.png")
        #plt.close()

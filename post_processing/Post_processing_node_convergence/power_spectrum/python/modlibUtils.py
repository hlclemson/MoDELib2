...
class PolyCrystalFile(dict):
    ...
    solidSolutionNoiseFile_xz=''
    solidSolutionNoiseFile_yz=''
...
#        L=np.zeros((3,3))
    ...
    def write(self):
        ...
        polyFile.write(f'solidSolutionNoiseFile_xz={self.solidSolutionNoiseFile_xz};\n');
        polyFile.write(f'solidSolutionNoiseFile_yz={self.solidSolutionNoiseFile_yz};\n');
        polyFile.write(f'stackingFaultNoiseMode={self.stackingFaultNoiseMode}; # 0=no noise, 1=correlated noise, 2=white noise\n');
        ...
        polyFile.write(f'a_cai_A={self.a_cai_A}; # add comment\n');
        polyFile.write(f'seed={self.seed}; # add comment\n');
        polyFile.close()
...
def readEVLtxtPlus(filename):
    ...
    numPolyIncEdges=int(evlFile.readline().rstrip())
    evl=EVL();
    #evl.nodeID=np.empty(numNodes, dtype=np.int64)
    #evl.nodePos=np.empty([numNodes, 3], dtype=np.float64)
    #for k in np.arange(numNodes):
    #    data=np.fromstring(evlFile.readline().rstrip(), sep=' ')
    #    evl.nodeID[k] = data[0]
    #    evl.nodePos[k,:]=data[1:4]
    evl.nodes=np.empty([numNodes, 4])
    for k in np.arange(numNodes):
        data=np.fromstring(evlFile.readline().rstrip(), sep=' ')
        evl.nodes[k,:]=data[0:4]

    # init connectivity data
    loopLinkDataSpan = 5
    evl.loopLink=np.empty([numNodes, loopLinkDataSpan], dtype=np.int64)
    ...
    for m in range(numNodes):
        while True:  # Keep reading until we get a valid line
            ...
            # End of file check
            if not line:
                raise ValueError(f"Reached EOF before reading {numNodes} nodes")
            data = np.fromstring(line.rstrip(), sep=' ')
            if len(data) == loopLinkDataSpan:
                evl.loopLink[m] = data
...
def readEVLtxtLoopNode(filename: str) -> EVL:
    ...
    for n in range(numLoops):
        while True:  # Keep reading until we get a valid line
            ...
            # End of file check
            if not line:
                raise ValueError(f"Reached EOF before reading {numLoops} nodes")
            data = np.fromstring(line.rstrip(), sep=' ')
...
#0	 4.999999999999998e-01  2.886751345948129e-01 -1.186412856211894e-17	1.000000000000000e+00 0.000000000000000e+00 5.551115123125781e-17	-4.999999999999999e-01 -0.000000000000000e+00 -2.775557561562890e-17	1	1	0.000000000000000e+00	4.383859745155614e+03	0.000000000000000e+00	3.075914498161468e+05
def readEVLtxtDislocLoop(filename):
    evlFile = open(filename+'.txt', "r")
    numNodes=int(evlFile.readline().rstrip())
    numLoops=int(evlFile.readline().rstrip())
    evl=EVL();
    # init connectivity data
    dislocLoopDataSpan = 16
    evl.loopNodes=np.empty([numLoops, dislocLoopDataSpan], dtype=np.float64)
    # Read file line by line until we fill the array
